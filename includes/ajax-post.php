...
function hg_stripe_donate_post()
{
	if( isset($_POST['action']) AND $_POST['action'] == 'hg-stripe-post' )
	{
		if( !class_exists('Stripe') )
		{
			...
			{
				hg_stripe_donate_error( sprintf( __( 'Credit Card Error: %s', 'hg-stripe-donate' ), $e->getMessage() ) );
			}
			
			hg_stripe_donate_error( __('Donation system is down at this time, please try again soon', hg-stripe-donate) );
		}	
		else
		{
			hg_stripe_donate_error(  __('Donation system is down at this time, please try again soon', hg-stripe-donate) );
